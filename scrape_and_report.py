...
def login_with_session(page, session_json_raw):
    """Inietta la sessione catturata dal browser reale dell'utente, forzando
    la scadenza a 0 così il client Supabase del sito la rinnova subito
    all'avvio — stesso meccanismo verificato all'inizio di questo progetto."""
    ...
    if "Sign In" in title or "sign-in" in page.url:
        raise RuntimeError(
            "La sessione non ha funzionato (probabilmente scaduta o già usata altrove). "
            "Rifai login su Cryptonary e riclicca il segnalibro per generarne una fresca."
        )
